...
class Extension extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      data = await ajax({
        url:
          'https://clients2.google.com/service/update2/crx?prodversion=' +
          getChromeVersion() +
          '&x=id%3D' +
          id +
  ...
  render() {
    ...
    if (!extension) {
      return (
        <ExtensionDiv display='flex'>
          <Extensiony id='loader' color={shared.themeMainColor} />
        </ExtensionDiv>
      );
    }
    let state =
      extension.state || (extension.enabled ? GL('enabled') : GL('disabled'));
    ...
    if (state != 'removed') {
      ...
      if (extension.type != 'theme') {
        switchy = (
          <Switchy
            onClick={() => {
              sendMessage(
                { job: 'extensionToggle', id: extension.id },
                () => {}
              );
            }}
            color={shared.themeMainColor}
      ...
      if (extension.optionsUrl && extension.optionsUrl.length > 0) {
        optioney = (
          <Optioney
            onClick={sendMessage.bind(
              null,
              { job: 'extensionOptions', id: extension.id },
              () => {}
            )}
            color={shared.themeMainColor}
          />
      ...
      removy = (
        <Removy
          onClick={sendMessage.bind(
            null,
            { job: 'extensionRemove', id: extension.id },
            () => {}
          )}
          color={shared.themeMainColor}
        />
      ...
      chromey = (
        <Chromey
          onClick={sendMessage.bind(
            null,
            { job: 'extensionBrowserOptions', id: extension.id },
            () => {}
          )}
          color={shared.themeMainColor}
        />
      ...
      addy = (
        <Addy
          onClick={sendMessage.bind(
            null,
            { job: 'openWebStore', id: extension.id },
            () => {}
          )}
          color={shared.themeMainColor}
        />
    ...
    if (extension.launchType) {
      launchType = (
        <tr>
          <td>{GL('launch_type')}</td>
          <td>{extension.launchType}</td>
        </tr>
      );
    }
    ...
    hostPermissions = (
      <tr>
        <td>{GL('host_permissions')}</td>
        <td>
          <ul>{hostPermissionList}</ul>
        </td>
      </tr>
    );
    ...
    if (this.state.joinCommunity) {
      extensionTags = (
        <tr>
          <td>{GL('nooboss_tags')}</td>
          <td>
            <div id='tags'>
              <div className='tagColumn'>
                <div
                  onClick={this.toggleTag.bind(this, 'useful')}
                ...
                <div
                  onClick={this.toggleTag.bind(this, 'working')}
              ...
              <div className='tagColumn'>
                <div
                  onClick={this.toggleTag.bind(this, 'laggy')}
                ...
                <div
                  onClick={this.toggleTag.bind(this, 'buggy')}
              ...
              <div className='tagColumn'>
                <div
                  onClick={this.toggleTag.bind(this, 'not_working')}
                ...
                <div
                  onClick={this.toggleTag.bind(this, 'ASM')}
    ...
    return (
      <ExtensionDiv>
        <div id='actions'>
          <a
            id='icon'
            title={'https://chrome.google.com/webstore/detail/' + extension.id}
            target='_blank'
        ...
        <table id='appDetail'>
          <tbody>
            <tr>
              <td>{GL('last_update')}</td>
              <td>
                <TimeAgo
                  datetime={extension.lastUpdateDate}
                  locale={this.props.language}
                />
              </td>
            </tr>
            <tr>
              <td>{GL('first_installed')}</td>
              <td>
                <TimeAgo
                  datetime={extension.installedDate}
                  locale={this.props.language}
                />
              </td>
            </tr>
            <tr>
              <td>{GL('enabled')}</td>
              <td>{capFirst(extension.enabled)}</td>
            </tr>
            <tr>
              <td>{GL('homepage_url')}</td>
              <td>
                <a
                  title={extension.homepageUrl}
                  target='_blank'
            ...
            <tr>
              <td>{GL('short_name')}</td>
              <td>{extension.shortName}</td>
            </tr>
            <tr>
              <td>{GL('type')}</td>
              <td>{capFirst(extension.type)}</td>
            </tr>
            {launchType}
            <tr>
              <td>{GL('offline_enabled')}</td>
              <td>{capFirst(getString(extension.offlineEnabled))}</td>
            </tr>
            <tr>
              <td>{GL('download_crx')}</td>
              <td>
                <a
                  title={this.state.crxUrl}
                  target='_blank'
                  href={this.state.crxUrl}>
                  {this.state.crxName}
                </a>
              </td>
            </tr>
            <tr>
              <td>{capFirst('update_url')}</td>
              <td>
                <a
                  title={extension.updateUrl}
                  target='_blank'
                  href={extension.updateUrl}>
                  {extension.updateUrl}
                </a>
              </td>
            </tr>
            <tr>
              <td>{capFirst('manifest_file')}</td>
              <td>
                <a
                  target='_blank'
                  onClick={sendMessage.bind(
                    null,
                    { job: 'openManifest', id: extension.id },
                    () => {}
                  )}
                  href={manifestUrl}
                  title={manifestUrl}>
            ...
            <tr>
              <td>{capFirst('may_disable')}</td>
              <td>{capFirst(getString(extension.mayDisable))}</td>
            </tr>
            <tr>
              <td>{capFirst('install_type')}</td>
              <td>{capFirst(extension.installType)}</td>
            </tr>
            {hostPermissions}
            {permissions}
          </tbody>
