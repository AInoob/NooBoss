...
class History extends Component {
  ...
  emptyHistory() {
    alerty(GL('are_you_sure'), shared.themeMainColor, () => {
  ...
  render() {
    ...
    const recordList = this.props.recordList
      ...
      .filter((elem, index) => index < this.state.maxRecords)
      .map((record, index) => {
        return (
          <tr key={index} className='record'>
            <td>
              <TimeAgo datetime={record.date} locale={this.props.language} />
            </td>
            <td>{GL(record.event)}</td>
            <td className='icon'>
              <img src={icons[record.icon]} />
            </td>
            <td
              className='name'
              onClick={this.props.updateSubWindow.bind(
                null,
                'extension',
                record.id
              )}>
    ...
    return (
      <HistoryDiv themeMainColor={window.shared.themeMainColor}>
        <div id='emptyHistoryDiv' className='line'>
