  ...
  return {
    ...
    manage: async (tabId) => {
      ...
      for (let i = 0; i < rules.length; i++) {
        ...
        const pattern = rule.match.isWildcard
          ? getRegExpFromWildcard(rule.match.url)
          : new RegExp(rule.match.url, 'i');
      ...
      let x = await Promise.all(
        idList.map((id) => {
          ...
          return NooBoss.AutoState.setAppState(
            id,
            phase.enabled,
            phase.tabId,
            phase.ruleId
          ...
          if (appInfo && appInfo.enabled != enabled) {
            ...
            isOn('autoStateNotification', async () => {
              notify(
                GL('autoState'),
                GL('x_7')
                  .replace('X1', appInfo.name)
                  .replace(
                    'X2',
                    GL(enabled ? 'is_enabled' : 'is_disabled').toLowerCase()
                  )
                  .replace('X3', ruleId + 1),
